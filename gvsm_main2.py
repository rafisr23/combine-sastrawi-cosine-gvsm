...
def getStemming(kalimat):
    ...
    remove_punctuation = remove_number.translate(
    ...
    hasil_tokens = ''
    for x in tokens:
        # Memasukkan kata yang sudah dipisah (list), disatukan kembali menjadi string
        hasil_tokens += ' ' + x

    # Menghapus kata Stopword - Sastrawi
    stop_factory = StopWordRemoverFactory().get_stop_words()  # load default stopword
    more_stopword = ['vol', 'volume', 'issn', 'php',
                     'mysql', 'gunadarma']  # menambahkan stopword

    data = stop_factory + more_stopword  # menggabungkan stopword

    dictionary = ArrayDictionary(data)
    str_stopword = StopWordRemover(dictionary)
    hasil_stopword = nltk.tokenize.word_tokenize(
        str_stopword.remove(hasil_tokens))
...
def similarity(vector_query, dict_list):
    ...
    for i in range(len(dict_list[0])):
        atas = sum([vector_query[x] * dict_list[x][i]
...
def show_result(list_of_documents, results):
    for i in range(len(results)):
        if results[i] > 0:
            print("Document ke-" + str(i+1) +
                  " memiliki similarity sebesar: " + str(results[i]))
        else:
...
def main():
    ...
    path = 'D:\ITENAS\Semester 5\Data Mining\similarity index\daftar-file'
    files = []
